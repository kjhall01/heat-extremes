#!/usr/bin/env python
"""Write ERA5 T2M min/mean/max matched to an AIFS forecast-day store."""
...
def main() -> None:
    ...
    observations = sample_observations_at_forecast_day_leads(
        era5["2m_temperature"], model.time, forecast_days
    )
    daily = aggregate_6hourly_observation_samples(observations)
    daily = normalize_longitude(daily).reindex(
        latitude=model.latitude,
        longitude=model.longitude,
    )
    daily = daily.chunk(
        {"time": 4, "forecast_day": 1, "latitude": 180, "longitude": 180}
    )
    daily.attrs.update(
        {
    ...
    print("Matched ERA5 aggregate store complete")
