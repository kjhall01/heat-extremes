...
#
# Prefix changed from "aifs_singlev2_" to "aifs_ensv2_mean_" when the model
# source switched from AIFS-single-v2's own run to AIFS-ENS-v2's ensemble
# mean -- deliberately, not cosmetically: keeping the old prefix would have
# let a stale cache file from the old (different!) data source get silently
# reused for this new one, since test_year_start/end/region_label/variable/
# threshold_label alone wouldn't have changed.
by_lead_map_cache_path = Path(
    f"aifs_ensv2_mean_by_lead_map_{test_year_start}_{test_year_end}_{region_label}_{variable}_{threshold_label}.nc"
)