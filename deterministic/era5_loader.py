...
Self-contained: does not import the ``heatextremes`` package. Copied
unchanged from ``heatextremes.open_cached_era5`` -- that source file has no
internal dependency on the rest of the ``heatextremes`` package (only
``pathlib``, ``numpy``, ``xarray``), so relocating it here is a straight
copy, not a reimplementation. Done so this notebook's pipeline doesn't
depend on the ``heatextremes`` package at all, since that package is a
shared, read-only dependency here.
"""
...
def daily_era5_aggregates(ds: xr.Dataset) -> xr.Dataset:
    ...
    ``t2m_min_6h`` (daily minimum, alongside the original ``t2m_mean_6h``/
    ``t2m_max_6h``) was added here to support running the same
    POD/FAR/SEDI verification against daily min/max temperature, not just
    the daily mean -- so this is no longer an unmodified copy of the
    upstream ``heatextremes`` version, just the same resampling pattern
    extended by one more reduction.
    """
